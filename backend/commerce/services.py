...
def checkout(*, user, shift_id, location_id, customer_id, items, payments, idempotency_key):
    ...
    location = InventoryLocation.objects.filter(id=location_id, active=True).first()
    if not location:
        raise ValidationError('Active inventory location not found.')
    ...
    sale = Sale.objects.create(
        number=f'TK-{uuid4().hex[:10].upper()}', customer=customer, cashier=user,
        location=location, shift=shift, subtotal=subtotal, total=total,
